...
class _UserModel(nn.Module):
    ...
    def forward(self, uids, u_item_pad, u_user_pad, u_user_item_pad):
        ...
        # uids에 user_emb을 입혀주고 -> q_a item embedding q_a 사이즈로 맞춰줌
        ...
        # Normalizing process
        # (Batch_size, max_len) : batch_size의 유저수, 해당 유저들과 아이템들의 attention score
        ...
        # item_emb : nn.Embedding(self.num_items, self.emb_dim, padding_idx =0)
        # u_users_items_list : user별로 trust 하는 user들이 interaction한 (item, rating)
        ...
        # 해당 u_i기준으로 interaction이 있는 user들을 임베딩 해서 emb_dim으로 만듦
...
class _ItemModel(nn.Module):
    ...
    def forward(self, iids, i_user_pad):
        ...
        # i_users_list: 각 item별로 user가 rating을 준 list
        # (user_id, rating)
